...
Переходим в директорию, куда установили компонент Liferay. Далее перемещаемся в папку `wars`. Именно здесь мы будем создавать новые портлеты. Теперь запускаем `cmd` и вписываем следующие команды:
```sh
